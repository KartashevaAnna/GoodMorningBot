"""Не удается найти, откуда импортировать ReplyKeyboardBuilder, поэтому я скопировала исходный год с гитхаба"""
...
class KeyboardBuilder(Generic[ButtonType]):
    ...
    def _validate_row(self, row: List[ButtonType]) -> bool:
        ...
        if len(row) > MAX_WIDTH:
            raise ValueError(
                f"Row {row!r} is too long (MAX_WIDTH={MAX_WIDTH})"
            )
    ...
    def copy(
        self: "KeyboardBuilder[ButtonType]",
    ...
    def row(
        ...
        self._validate_buttons(*buttons)
        self._markup.extend(
            list(buttons[pos : pos + width])
            for pos in range(0, len(buttons), width)
        )
        return self
    ...
    def adjust(
        ...
        sizes_iter = (
            repeat_all(validated_sizes)
            if repeat
            else repeat_last(validated_sizes)
        )
    ...
    def button(self, **kwargs: Any) -> "KeyboardBuilder[ButtonType]":
        if isinstance(
            callback_data := kwargs.get("callback_data", None), CallbackData
        ):
