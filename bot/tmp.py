...
kb.add(KeyboardButton("/help")).insert(KeyboardButton("/orange")).add(
...
async def command_desc(message: types.Message):
    await bot.send_message(
        chat_id=message.from_user.id, text="Наш бот просто прикольный!"
    )
...
async def send_random(message: types.Message):
    await bot.send_location(
        chat_id=message.chat.id, latitude=randrange(1, 100), longitude=randrange(1, 100)
    )
